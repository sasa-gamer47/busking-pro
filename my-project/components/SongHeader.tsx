...
import { ArrowLeft, Settings, X, Sliders } from "lucide-react";
...
interface SongHeaderProps {
  ...
  onUpdate?: (data: { bpm: number; currentKey: string; capo: number; tuning: string }) => void; // <-- Aggiunto il "?"
}
...
export default function SongHeader({
  ...
  // Inizializzazione con fallback stringa vuota o valori standard per evitare l'undefined
  ...
  // Sincronizza e inserisce i dati correntemente esistenti all'apertura del Modal
  ...
  const handleSave = () => {
  // Controlla che onUpdate esista e sia effettivamente una funzione prima di eseguirla
    if (typeof onUpdate === "function") {
      ...
      console.warn("BuskingPro Warning: onUpdate non è stata passata a <SongHeader />");
    }
  ...
  return (
    <>
      <header className="col-start-1 col-end-2 row-start-1 row-end-2 border-b border-zinc-900 bg-zinc-950 px-6 flex items-center justify-between h-20">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-xl font-bold text-zinc-200 truncate max-w-[300px] sm:max-w-md">
              {title} {artist && <span className="text-zinc-500 text-sm font-normal">/ {artist}</span>}
            </h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 text-zinc-400">
          <Button
            ...
            variant="outline"
            className="border-zinc-800 bg-zinc-950 text-zinc-400 hover:bg-zinc-900 hover:text-white h-10 px-3.5 rounded-xl gap-x-2 cursor-pointer transition"
          >
          ...
          <div className="bg-zinc-950 border border-zinc-800 rounded-2xl w-full max-w-md p-6 shadow-2xl relative flex flex-col gap-y-5">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-zinc-500 hover:text-zinc-300 transition p-1 rounded-lg hover:bg-zinc-900"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex flex-col gap-y-4">
              ...
              {/* Campo BPM con controllo stringa vuota per il backspace fluido */}
              ...
              <div className="flex flex-col gap-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-lg text-sm p-2.5 text-zinc-200 outline-none focus:border-orange-500 h-10 cursor-pointer"
                >
                  {TONALITIES.map((k) => (
              ...
              <div className="flex flex-col gap-y-1.5">
                ...
                <select
                  value={localCapo}
                  onChange={(e) => setLocalCapo(Number(e.target.value))}
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-lg text-sm p-2.5 text-zinc-200 outline-none focus:border-orange-500 h-10 cursor-pointer"
                >
                  {Array.from({ length: 13 }, (_, i) => (
                    <option key={i} value={i}>
                      {i === 0 ? "Nessun Capo (0)" : `Tasto ${i}`}
                    </option>
                  ))}
                </select>
              </div>

              {/* Dropdown Accordatura */}
              <div className="flex flex-col gap-y-1.5">
            ...
            <div className="flex items-center justify-end gap-x-2 mt-1">
              <Button
                variant="ghost"
                onClick={() => setIsModalOpen(false)}
                className="text-zinc-400 hover:text-white hover:bg-zinc-900 rounded-lg cursor-pointer text-xs"
              >
                Annulla
              </Button>
              <Button
                onClick={handleSave}
                className="bg-orange-600 hover:bg-orange-700 text-white font-semibold rounded-lg text-xs px-4 h-9 transition cursor-pointer"
              >
                Applica Modifiche
              </Button>
            </div>
